...
namespace MFST
{
	...
	Mfst::Mfst(std::vector<LEXER::Lexem> lexems, LEXER::IdTable table, GRB::Greibach pgrebach)
	{
		grebach = pgrebach;
		this->table = table;
		this->lexems = lexems;
		lenta = new short[lenta_size = lexems.size()];//размер ленты = текущий размер таблицы лексем
		for (int k = 0; k < lenta_size; k++)lenta[k] = TS(lexems[k].lexema[0]);//заносит в ленту терминалы
		lenta_position = 0;
	...
	Mfst::RC_STEP Mfst::step(const std::shared_ptr<std::ofstream> log) //выполнить шаг автомата
	{
	...
	bool Mfst::savestate(const std::shared_ptr<std::ofstream> log) //сохранить состояние автомата
	{
	...
	bool Mfst::reststate(const std::shared_ptr<std::ofstream> log) //восстановить состояние автомата
	{
		bool rc = false;
		MfstState state;
	...
	bool Mfst::savediagnois(RC_STEP prc_step)
	{
		bool rc = false;
		short k = 0;
		while (k < MFST_DIAGN_DIGBER && lenta_position <= diagnosis[k].lenta_position)k++;
		if (rc = (k < MFST_DIAGN_DIGBER))
		{
			diagnosis[k] = MfstDiagnosis(lenta_position, prc_step, nrule, nrulechain);
			for (short j = k + 1; j < MFST_DIAGN_DIGBER; j++)diagnosis[j].lenta_position = -1;
		};
		return rc;
	};
	bool Mfst::start(const std::shared_ptr<std::ofstream> log)
	{
		bool rc = false;
		...
		case LENTA_END:         MFST_TRACE4(log, "------>LENTA_END")
			* log << "-------------------------------------------------------------------------------------" << std::endl;
			sprintf_s(buf, 1024, "%d:всего строк %d, синтаксический анализ выполнен без ошибок ", 0, lenta_size);
			*log << std::setw(4) << std::left << 0 << ":всего строк " << lenta_size << ", синтаксический анализ выполнен без ошибок " << std::endl;
			rc = true;
			break;
		case NS_NORULE:         MFST_TRACE4(log, "------>NS_NORULE")
			* log << "-------------------------------------------------------------------------------------" << std::endl;
			*log << getDiagnosis(0, buf) << std::endl;
			*log << getDiagnosis(1, buf) << std::endl;
			*log << getDiagnosis(2, buf) << std::endl;
			break;
	...
	const char* Mfst::getDiagnosis(short n, char* buf)
	{
		...
		if (n < MFST_DIAGN_DIGBER && (lpos = diagnosis[n].lenta_position) >= 0)
		{
			...
			ERROR_LOG(std::format("Sourse code: строка {}, индекс лексемы {}", currentLexem.line, currentLexem.index),
					  std::format("Неверная структура программы"));
			throw "Esception";
		};
		return rc;
	};
	void Mfst::printrules(const std::shared_ptr<std::ofstream> log)
	{
		*log << endl;
		*log << endl;
		*log << "-------------------Дерево разбора-------------------------- " << endl;
		MfstState state;
		GRB::Rule rule;
