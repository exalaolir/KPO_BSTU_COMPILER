...
namespace GEN
{
    ...
    void Generator::Generate()
    {
        ...
        code.push_back(MAKE_VAR("byte_err", types[LEXER::String], "\"Ошибка, повреждение типа byte\", 0") + "\n");
    ...
    void Generator::GenerateExpression(std::list<std::string>& code, size_t& index, bool isReturn, size_t ind, char end, int iterRebase)
    {
        ...
        auto count = [&](auto operators, bool isDouble = false, bool isReturn = false, string end = ";", bool isString = false)
            {
                while (lexTable[index].originalText != end)
                {
                    ...
                    {
                        expression.push_back(CALL(MAKE_NAME(currentFun), currentFun.valueType == Double));
                        doubleFlag = currentFun.valueType == Double;
                    ...
                    else if (lexTable[index].lexema == "o")
                    {
                        ...
                        bool isFun = lexTable[index + 1].lexema == "i" && idTable[lexTable[index + 1].positionInIdTable].type == Fun;
                        bool param = lexTable[index + 1].originalText == "$" || isFun;
                        if (param && doubleFlag)
                        {
                            expression.push_back(PUSH_REAL_PARAM);
                        }
        ...
        else if (var.valueType == Byte || var.valueType == ByteLiteral)
        {
    ...
    void Generator::GenerateIf(std::list<std::string>& code, size_t& index, size_t localIfCounter, bool rebase)
    {
        ...
        while (isWork)
        {
            ...
            case 'w':
                index += 2;
                GenerateWhile(code, index, whileCounter, false);
                break;
            default:
                break;
            }
    ...
    void Generator::GenerateWhile(std::list<std::string>& code, size_t& index, size_t localIfCounter, bool rebase)
    {
        ...
        while (isWork)
        {
            ...
            case 'w':
                index += 2;
                GenerateWhile(block, index, whileCounter, false);
                break;
            default:
                break;
            }
